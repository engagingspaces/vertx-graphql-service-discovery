...
public class SchemaDefinition implements Queryable {
	...
	protected SchemaDefinition(GraphQLSchema schema, SchemaMetadata metadata) {
		...
		this.serviceAddress = schemaMetadata.getServiceAddress() == null || schemaMetadata.getServiceAddress().isEmpty()
		...
		schemaMetadata.put(SchemaMetadata.METADATA_MUTATIONS,
				!schema.isSupportingMutations() ? Collections.emptyList()
						: schema.getMutationType().getFieldDefinitions().stream().map(GraphQLFieldDefinition::getName)
	 ...
	 * <p>
	 * On success a {@link QueryResult} is returned. While this indicates the
	 * query executor has finished processing, the query itself might still have
	 * failed, so be sure to check {@link QueryResult#isSucceeded()} and
	 * {@link QueryResult#getErrors()} properties on the query result
	 * afterwards. afterwards.
	 *
	 * @param graphqlQuery
	 *            the graphql query
	 * @param resultHandler
	 *            the result handler with the query result on success, or a
	 *            failure
	 */
	@Override
	...
	/**
	 * Executes the GraphQL query on the GraphQL schema proxy using the provided
	 * variables.
	 *
	 * @param graphqlQuery
	 *            the graphql query
	 * @param resultHandler
	 *            the result handler with the graphql query result on success,
	 *            or a failure
	 */
	@Override
	public void queryWithVariables(String graphqlQuery, JsonObject variables,
		...
		try {
			queryBlocking(graphqlQuery, variables, resultHandler);
	 ...
	 * <p>
	 * A valid schema instance must be available on {@link Queryable} service
	 * proxy implementations that will be published. Accessing this method from
	 * a service proxy results in an {@link UnsupportedOperationException}.
	 *
	 * @return the graphql schema to be published and queried
	 * @throws UnsupportedOperationException
	 *             if invoked from a service proxy
	 */
	...
	/**
	 * Executes a blocking call to the GraphQL query processor and executes the
	 * query.
	 *
	 * @param graphqlQuery
	 *            the graphql query
	 ...
	 */
	public void queryBlocking(String graphqlQuery, JsonObject variables,
			Handler<AsyncResult<QueryResult>> resultHandler) {
		Objects.requireNonNull(graphqlQuery, "GraphQL query cannot be null");

		graphql.async.GraphQL graphQL = graphql.async.GraphQL.newAsyncGraphQL(schema()).build();

		Consumer<ExecutionResult> consumer = new Consumer<ExecutionResult>() {
		...
		if (variables == null) {
			graphQL.execute(graphqlQuery, consumer);
		} else {
			graphQL.execute(graphqlQuery, (Object) null, variables.getMap(), consumer);
		}
	}

	/**
	 * Creates a new {@link QueryResult} data object from the provided GraphQL
	 * {@link ExecutionResult}.
	 *
	 * @param executionResult
	 *            the execution result of the GraphQL query
	...
	public static QueryResult convertToQueryResult(ExecutionResult executionResult) {
		...
		return new QueryResult(
				...
				succeeded, executionResult.getErrors().stream().map(SchemaDefinition::convertToQueryError)
						.collect(Collectors.toList()));
	}

	/**
	 * Creates a new {@link QueryError} data object based on the provided
	 * {@link GraphQLError}.
	 *
	 * @param graphQLError
	 *            the graphql error to convert
	...
	public static QueryError convertToQueryError(GraphQLError graphQLError) {
		return new QueryError(graphQLError.getErrorType().name(), graphQLError.getMessage(),
				graphQLError.getLocations().stream()
						.map(location -> new ErrorLocation(location.getLine(), location.getColumn()))
						.collect(Collectors.toList()));
	}
}
